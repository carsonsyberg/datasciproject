...
int main(int argc, char const *argv[]) {
    ...
    for(int i = 0; i < 100; i++) {
        ...
        for(int j = 0; j < 100; j++) 
            doubleList.search(testData[randIndex[j]]); // starts by searching for value at 1st random generated index, then next and so on
    ...
    // adds time data to external files (first column insertTimes, second column searchTimes)
    for(int i = 0; i < 100; i++) {
            outputFile << insertTime[i] << "," << searchTime[i] << endl;
    }
