...
int main(int argc, char const *argv[]) {
    ...
    for(int i = 0; i < 100; i++) {
        ...
        HeapSort theHeap = HeapSort(insertIndexEnd+1);
        
        for(int i = 0; i <= insertIndexEnd; i++) {
            theHeap.insert(testData[i]);
        }
        
        auto start = chrono::steady_clock::now(); // gets current time
        for(int j = 0; j <= insertIndexEnd; j++) // adding 100 elements to array first loop 0-99 then 100-199 then 200-299 etc...
            theHeap.heapify(j);
